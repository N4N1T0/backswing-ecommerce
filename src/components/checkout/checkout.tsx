...
const Checkout = ({ total, count, shipping }: { total: number, shipping: number, count: CartItem[] }) => {
  return (
      ...
      <div className='p-6 border bg-gray-100 border-gray-300 md:p-8'>
        ...
        <div className='flex items-center justify-between pb-4 mb-4 '>
          ...
          {count.length === 0
            ...
            : (
              <span className='text-xl font-bold text-gray-700 '>{shipping}</span>
            )}
        </div>
        <div className='flex items-center justify-between pb-4 mb-4 '>
          <span className='text-gray-700 font-semibold'>Total</span>
          <span className='text-xl font-bold text-gray-700'>{total + shipping}</span>
        </div>
