...
const WishlistHeart = ({ product }: { product: StaticProductsTypes }) => {
  ...
  return (
    <Heart size={20} fill={isWishlisted ? 'red' : 'white'} onClick={() => { setCount(prev => [...prev, product]) }} className='cursor-pointer hover:text-gray-400 text-gray-950 transition-colors duration-200 ease-out' />
  )
}
