 ...
 */
export const accordionPathname = (pathname: string, labels: string[]): string[] => {
 ...
 */
export const pathnameCrumbs = (pathname: string): any[] => {
  // Split the pathname by '/' and remove any empty strings
  const crumbs = pathname.split('/').filter(Boolean)
...
export const calculateTotal = (count: CartItem[]) => {
    return count.map(item => {
        ...
        return parseFloat(priceWithoutCommas) * item.quantity
    }).reduce((a, b) => a + b, 0)
}
